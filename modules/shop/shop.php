				...
				if (array_key_exists($uid, $cart) && count($cart[$uid]['variations']) > 0)
					foreach ($cart[$uid]['variations'] as $variation_id => $properties) {
						...
						$result['cart'][] = array(
							...
							'discount_price'  => $item->discount ? $properties['price'] * ((100 - $item->discount) / 100), 2 : $properties['price'],
							'tax'             => $item->tax,
							'image'           => $thumbnail_url,
							'uid'             => $item->uid,
	...
	private function json_AddItemToCart() {
		...
		if (is_object($item)) {
			...
			$result = array(
				...
				'price'           => $item_price,
				'discount'        => $item->discount,
				'discount_price'  => $item->discount ? $item_price * ((100 - $item->discount) / 100), 2 : $item_price,
				'tax'             => $item->tax,
				'size_definition' => $item->size_definition,
				'image'           => $thumbnail_url,
